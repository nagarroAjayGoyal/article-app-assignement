...
describe("rendering Dashboard", () => {
  ...
  it("Allows selecting days in dropdown and updates articles", () => {
    render(
      <Provider store={store}>
        <Dashboard />
      </Provider>,
    ...
    // Mock select dropdown and change value
    const dropdown = screen.getByRole("combobox");
    fireEvent.change(dropdown, { target: { value: "7" } });

    expect(
      screen.getByText(/20 Popular Articles From Last 7/i)
    ).toBeInTheDocument(); // Verify updated dropdown header text
  });
});
